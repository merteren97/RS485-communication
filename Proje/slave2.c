...
void iletisim()
{
   ...
   if(buffer_rx[1] == SLAVE2)
   {
      ...
      if(buffer_rx[1] == SLAVE2 && Error == FALSE)
         new_packet = TRUE;
...
void toggle_sag()
{
   ...
   create_packet(SLAVE2,0x07,1,0); // ACK
}
   ...
   while(TRUE)
   {
      ...
      if(new_packet){    
         switch(buffer_rx[3])
         {
            case 0x01:
               delay_ms(20);
               create_packet(SLAVE2,0x01,1,0);
               tam_tur_sag();
               break;
            case 0x02:
               delay_ms(20);
               create_packet(SLAVE2,0x02,1,0);
               tam_tur_sol();
               break;
            case 0x03:
               delay_ms(20);
               create_packet(SLAVE2,0x03,1,0);
               yarim_tur_sag();
               break;
            case 0x04:
               delay_ms(20);
               create_packet(SLAVE2,0x04,1,0);
               yarim_tur_sol();
               break;
            case 0x05:
               delay_ms(20);
               create_packet(SLAVE2,0x05,1,0);
               toggle_sag();
               break;
            case 0x06:
               delay_ms(20);
               create_packet(SLAVE2,0x06,1,0);
               toggle_sol();
               break;
            case 0x07:
               delay_ms(20);
               create_packet(SLAVE2,0x07,1,0);
               break;
            case 0x08:
               delay_ms(20);
               create_packet(SLAVE2,0x08,1,0);
